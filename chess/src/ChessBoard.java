...
public class ChessBoard extends GridPane {
    ...
    Tile clicked;
    ArrayList<Piece> whiteCaptured; //white pieces that are captured
    ArrayList<Piece> blackCaptured; //black pieces that are captured
    ...
    Image white_queen_elim;
    Image white_bishop_elim;
    Image white_knight_elim;
    Image white_rook_elim;
    Image white_pawn_elim;

    Image black_queen_elim;
    Image black_bishop_elim;
    Image black_knight_elim;
    Image black_rook_elim;
    Image black_pawn_elim;
    ...
    private void initializePieceImages() throws FileNotFoundException {
        ...
        white_queen_elim = new Image(new FileInputStream(path + white + "_queen_elim.png"));
        white_bishop_elim = new Image(new FileInputStream(path + white + "_bishop_elim.png"));
        white_knight_elim = new Image(new FileInputStream(path + white + "_knight_elim.png"));
        white_rook_elim = new Image(new FileInputStream(path + white + "_rook_elim.png"));
        white_pawn_elim = new Image(new FileInputStream(path + white + "_pawn_elim.png"));

        black_queen_elim = new Image(new FileInputStream(path + black + "_queen_elim.png"));
        black_bishop_elim = new Image(new FileInputStream(path + black + "_bishop_elim.png"));
        black_knight_elim = new Image(new FileInputStream(path + black + "_knight_elim.png"));
        black_rook_elim = new Image(new FileInputStream(path + black + "_rook_elim.png"));
        black_pawn_elim = new Image(new FileInputStream(path + black + "_pawn_elim.png"));
    }
    ...
    public Image getElimIcon(Pc piece, Color color) {
        if (color == Color.WHITE) {
            switch (piece) {
                case KING: return null;
                case QUEEN: return white_queen_elim;
                case BISHOP: return white_bishop_elim;
                case KNIGHT: return white_knight_elim;
                case ROOK: return white_rook_elim;
                case PAWN: return white_pawn_elim;
                default: return null;
            }
        }
        if (color == Color.BLACK) {
            switch (piece) {
                case KING: return null;
                case QUEEN: return black_queen_elim;
                case BISHOP: return black_bishop_elim;
                case KNIGHT: return black_knight_elim;
                case ROOK: return black_rook_elim;
                case PAWN: return black_pawn_elim;
                default: return null;
            }
        }
        return null;
            ...
            if (tile.hasPiece() && tile.getPiece().getPc() == piece && tile.getPiece().getColor() == color) {
                if (tile.getPiece().getMoves(this).contains(this.getTile(destFile, destRank))) {
                    if (startFile != 0 || startRank != 0) {
                        if (tile.getFile() == startFile || tile.getRank() == startRank) {
                            if (move.contains("=")) {
                                tile.getPiece().setReadMovePromote(true);
                                System.out.print("---" + move.charAt(move.indexOf("=")+1) + "---");
                                tile.getPiece().setPromotePiece(move.charAt(move.indexOf("=")+1), tile.getPiece().getColor(), this.getTile(destFile, destRank));
                            }
    ...
    public void clearBoard() {
        ...
        blackTurn = false;
        if (lastMoveTo != null) {
            lastMoveTo.getTileSquare().setFill(lastMoveTo.getShade());
        }
        if (lastMoveFrom != null) {
            lastMoveFrom.getTileSquare().setFill(lastMoveFrom.getShade());
        }
        lastMoveFrom = null;
        lastMoveTo = null;
        for (Node node : this.getChildren()) {
    ...
    public void setLastMove(Tile from, Tile to) {
        if (lastMoveTo != null) {
            lastMoveTo.getTileSquare().setFill(lastMoveTo.getShade());
        }
        if (lastMoveFrom != null) {
    ...
    public void highlightLastMove() {
        if (lastMoveTo != null) {
            lastMoveTo.getTileSquare().setFill(Color.DARKSLATEGREY);
        }
        if (lastMoveFrom != null) {
            lastMoveFrom.getTileSquare().setFill(Color.SLATEGREY);
        }
    }
}
